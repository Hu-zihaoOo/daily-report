...
function searchExistingPage(callback) {
  const searchData = JSON.stringify({
    query: "Transformer 架构原理与实现",
    filter: {
      value: "page",
      property: "object"
    }
...
// Step 2: Delete all existing blocks (by replacing with empty)
function deletePageBlocks(pageId, callback) {
  const options = {
    hostname: 'api.notion.com',
  ...
  const req = https.request(options, (res) => {
    ...
    res.on('data', (chunk) => data += chunk);
    res.on('end', () => {
      console.log('Blocks cleared, status:', res.statusCode);
      callback();
    });
  });

  req.on('error', (e) => {
    console.error('Delete blocks error:', e);
    callback(); // Continue anyway
  });

  req.write(JSON.stringify({ children: [] }));
  req.end();
}
